...
export default function App() {
  ...
  useEffect(() => {
    fetchInitialData();
    // Auto-poll tasks every 10 seconds for real-time Telegram sync
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    await Promise.all([fetchTasks(), fetchConfig(), fetchStatus()]);
    setLoading(false);
  };
  ...
  const handleAddTask = async (columnId) => {
    ...
    try {
      const res = await fetch('/api/tasks', {
        ...
        body: JSON.stringify({ text, status: columnId }),
      });
      if (res.ok) {
  ...
  const getTasksByCol = (columnId) => {
    return activeTasks.filter((t) => t.status === columnId || (columnId === 'inbox' && !t.status));
  };

  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'radial-gradient(ellipse at top, #0f172a, #0b0f19)' }}>
      {/* Header */}
            ...
            <p style={{ fontSize: 12, color: 'hsl(var(--text-muted))', marginTop: 2 }}>
              ...
              {telegramStatus.tokenConfigured && telegramStatus.chatIdConfigured ? (
                ...
                <span style={{ color: 'hsl(var(--accent-amber))', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'hsl(var(--accent-amber))', display: 'inline-block', className: 'animate-pulse-slow' }}></span>
                ...
                <button
                  onClick={async () => {
                    if (confirm('This will delete all completed history permanently. Proceed?')) {
                      // Call delete for each or build an endpoint. 
                      // For simplicity, we delete them one by one.
                ...
                {digestEnabledInput && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingLeft: 30 }} className="animate-slide-in">
                    ...
                    <input
                      ...
                      style={{ background: 'hsl(var(--bg-input))', border: '1px solid hsl(var(--border-color))', color: '#fff', padding: '10px 14px', borderRadius: 'var(--border-radius-sm)', fontSize: 14, outline: 'none' }}
                    />
                    <p style={{ fontSize: 11, color: 'hsl(var(--text-muted))', marginTop: 2 }}>
                      Allows multiple trigger times in HH:MM format. For serverless (Vercel) hosting, you will also need to configure a Vercel cron job to hit the digest trigger route.
                    </p>
                  </div>
                )}
              </section>
